...
若 `machineScreening.status=pass`，还须提供独立的 `sermon-target-language-audio-screening-v1` JSON 收据，按顺序覆盖全部 group ID 与单元音频 hash、同一个 job 和 locale。机器筛查通过不会产生人工全文听审结论。正式完成还需三语真实 renderer、全文回转写筛查、自然音轨与视频 1 倍速人工审核，再据审核收据推进 `human_reviewed`。本构建器未生成 `audio_unavailable` 包，也未实现人工审核状态推进。
