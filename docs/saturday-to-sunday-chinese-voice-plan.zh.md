...
最新进展（2026-09-05）：用户认可 Eric 首轮和扩充训练样片；已制作另外五位讲员的独立训练音色与中文试听，App 增加讲员音色和制作进度选项卡。完整 8 月 30 日证道已通过新流程生成约 27 分钟中文候选，完成逐段回听检查与发音修订。见[周六配音扩展 Runbook](../experiments/sermon-dubbing-poc/SATURDAY_AUDIO_RUNBOOK.zh.md)和[Firebase App](https://ai-for-god-sermon-audio.web.app)。

![在周六流程上扩展讲员音色、配音、审核与周日播放](./diagrams/saturday-chinese-voice-workflow.svg)

状态：**Discovery / 可运行的候选生成与审核流程**。已确认声音训练、配音授权；新版整篇音频仍需人工试听与同视频同步审核。样本的 9 处边界和 15 段时长问题已列入复核清单，不能宣称现场同声传译已验收。
...
用户已确认：**周日播放周六提供的同一份英文证道视频。** 因此首版按证道内容、剪辑顺序及速度一致设计；后续需要确认的是现场播放器能否提供共同播放时钟。“同一主题”“同一讲员”“同一篇讲章”本身不满足同视频条件。
