...
状态：**9 月 20 日 2:58 片段的正式 Layer 1／2 已获人审，三语 Layer 3 speech job 已备齐；中文完整候选音轨已排入片段，韩／西音轨仍超时，三语音频听审和 Layer 4 Dev 发布待完成**。本 backlog 从已冻结的四层接口继续推进，覆盖：
...
- [x] 在授权 Spark 上按同一获批片段、原候选和语音身份生成三语全部单元。以已记录的自然偏快口播指令重跑后，中文 45、韩语 44、西语 44 个 WAV 均生成并完整解码；不同指令的渲染分别保留，未覆盖原合成。
- [x] renderer 对三语真实测量时长执行 1 倍速排程；超时时留下诊断，不生成成功 manifest。韩语原排程末端 203.32 秒，偏快指令加前导静音测量裁剪后为 182.78 秒；西语对应为 183.55 → 180.91 秒，仍分别超过片段 178.178 秒约 4.60 和 2.73 秒。
- [x] 中文偏快指令加前导静音测量裁剪后，45 个单元排入 178.178 秒视频，产出完整候选 track 与 render manifest。裁剪仅处理经 RMS 测量的前导静音，保存原 WAV、每单元原／新 SHA 和时长，以及独立裁剪收据；仍需回转录和人耳听审，不能据此宣布 Layer 3 通过。
- [ ] 韩／西在**不修改已批准文字、不拉伸播放、不延长获批片段**的前提下解决超时。可评估新自然口播指令或种子；若需压缩文字，回到 Layer 2 建立新 revision、独立模型复核和逐组人审，然后重新生成该语言的 Layer 3。
- [ ] 三语完整轨分别做全单元回转录、关键实体／经文抽查、1 倍速全文听审和与原视频同步审核；ASR 只产生机器筛查收据，不替代人工批准。
...
- [x] 提交不含媒体或密钥的[本片段 Layer 1／2 哈希与审核进度清单](evidence/2026-09-20-multilingual-clip-layer2.json)，明确三语 speech job 已备齐而完整音轨、全文听审和 Dev 发布尚未完成。
