...
- 周日实时字幕属于独立的 `live_session`，现场不要求生成四个预制生产包，也不能把 session finalization 报告为 `four_layer_release`。若录音以后要制作可复用的翻译、配音或页面，必须从 [Layer 1](multilingual-production-interfaces.zh.md) 重新建立来源与英文锚点。
