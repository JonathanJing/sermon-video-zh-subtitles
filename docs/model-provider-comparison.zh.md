...
| ASR 后文本翻译 | OpenAI `gpt-5.4-mini` | 高 | 段级翻译一般可控在数秒内，取决于 batching | 中文自然度、术语约束和结构化输出较稳 | 不能直接解决音频到字幕，需要 ASR |
...
|---|---:|---:|---|
| OpenAI `gpt-5.4-mini` | $0.75 / $4.50 per 1M tokens | $0.17-$0.23 | 适合稳定修正和离线高质量字幕 |
...
4. OpenAI `gpt-5.4-mini`：用英文 ASR cue 逐段或滑动窗口翻译。
...
3. 每 10 秒用 Gemini `gemini-3.1-flash-lite` 或 OpenAI `gpt-5.4-mini` 做稳定修正，优先修正经文、人名和术语。
...
4. API key 只存 Google Secret Manager，GCS artifact 只记录 secret resource name。
