...
1. `assemble_multilingual_hosting.py --production-reader --promote-home` 生成的已验证 Production 布局候选；样片 stage、v2 catalog、三语 release 与资源 SHA 都由上游冻结。这里读取它的公开目录，不调用 Production deploy。
...
- `tests.test_multilingual_dev_preview` 与相邻 Hosting 测试共 16 项通过；`node --check` 和 `git diff --check` 通过。模拟器的本地音频 Range 行为不能代替 Firebase 线上 206 验证。
