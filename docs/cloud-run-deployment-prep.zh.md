...
playback/VTT/SRT/manifest，但不能当作新的 `gpt-5.4-mini` API 调用成功证据。
...
`gpt-5.4-mini`、实时草稿 `gpt-realtime-translate`，并且没有 raw key material 或 Secret
...
gate 时，还必须至少有一条 `gpt-5.4-mini-stable-correction` 的 `caption_final`，并且没有
...
`gcloud run services update`，即使 `gpt-5.4-mini` access 或 Cloud Run realtime
...
caption delta 和一条 `gpt-5.4-mini` stable correction，再从
...
通过标准：报告是 `status=ok`，并且 `responses_model:gpt-5.4-mini`
...
`gpt-5.4-mini` stable/offline route 已满足。
