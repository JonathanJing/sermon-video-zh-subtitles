...
## 签名与实体设备增量（2026-09-06）

当前已完成实体 iPhone 17 Pro / iOS26.6.1 的18项 XCTest 回归，0失败、0跳过、无运行时警告，并保留真正的设备截图。普通 App 的真实声音、字幕、锁屏播放和解锁暂停经用户确认正常。Release Archive 及 App Store Connect IPA 导出均成功，签名验证通过；尚未上传或经过 Apple 审核。

原生 MCP 的工具发现仍超时，因此本轮实体自动化证据来自直接运行的 XCTest；它与下方较早的 Computer/MCP 界面阻塞分别记录。实际账号/签名/ASC 和隐私边界见 [Beta 记录](BETA-TESTING.zh.md)。
