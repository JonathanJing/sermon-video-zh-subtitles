...
/// Available offline; opening this page never requests the microphone or sends
/// feedback. External destinations are activated only by the user.
struct PrivacySupportView: View {
    @ObservedObject private var localization = AppLocalization.shared

    var body: some View {
        List {
            Section(localization.text("政策与联系")) {
                Link(localization.text("完整隐私政策"), destination: URL(string: "https://ai-for-god-tongxing-support.web.app/privacy.html")!)
                    .accessibilityIdentifier("full-privacy-policy")
                Link(localization.text("使用帮助与支持"), destination: URL(string: "https://ai-for-god-tongxing-support.web.app/support.html")!)
                    .accessibilityIdentifier("privacy-support-website")
            ...
            Section(localization.text("反馈与支持")) {
                Text(localization.text("TestFlight 会自动向 Apple 发送崩溃和使用信息并与开发者共享；你也可以主动发送反馈。请只提供问题时间和复现步骤，不要提交私人对话、凭据或其他敏感信息。Apple 按其 TestFlight 隐私规则处理这些资料。"))
                Link(localization.text("Apple TestFlight 隐私说明"), destination: URL(string: "https://www.apple.com/legal/privacy/data/en/test-flight/")!)
            }
        }
