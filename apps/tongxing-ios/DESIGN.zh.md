...
基础约定：2026-09-05；平台补充：2026-09-12。用户已选择 **iOS 27 设计语言**，继续使用“同行”、副标题“证道中文听译”和同行绿。设计目标是让听众容易读到当前字幕、找到播放控制并完成现场微调。
...
Duo 与灵动岛的 Apple 官方规范、同行内容分工和后续工具依赖见 [平台设计提炼](APPLE-PLATFORM-DESIGN.zh.md)；任务与验收条件见 [Backlog](BACKLOG.zh.md)。
...
iOS 26.5 SDK 可以用于验证这些接口的源码兼容性；采用 iOS 26 起提供的接口，也不等于已经验证 iOS 27 的实际外观或系统偏好行为。2026-09-12 已用 Xcode 27.0 完成部分 iOS 27 模拟器布局验证，见 [适配记录](DUO-ADAPTATION.zh.md)；这不关闭完整外观与真机检查。Duo 专用 SDK 与验收另见平台补充和 Backlog。
