...
def trimmed_samples(path: Path) -> tuple[bytes, int, float, float]:
    ...
    require(removed < len(values), "Trim would remove entire unit")
    remainder = values[removed:]
    if sys.byteorder != "little":
        remainder.byteswap()
    return remainder.tobytes(), rate, removed / rate, info["durationSeconds"]


def compact_unit(source_job: Path, destination_job: Path, index: int,
                 *, expected_job_hash: str) -> dict:
    ...
    compacted_bytes, rate, removed_seconds, original_duration = trimmed_samples(raw)
    ...
    return {"unitIndex": index, "textGroupId": unit["translationGroupId"],
...
def compact(paths: dict[str, Path], source_job: Path, destination_root: Path,
            checkpoint_map: Path, operation_policies: Path, *, path_map: Path | None = None) -> dict:
    ...
    receipt = {"schemaVersion": SCHEMA, "status": "measured_silence_removed",
    ...
    manifest = renderer.assemble(context, adapted_paths, destination_root,
                                 [row["manifestUnit"] for row in rows])
