...
During recording, `gateway_health_sample` records each completed 5-second health poll (including offline observations), with disk space, worker queues and publisher failure counters. Missing measurements remain null. Browser suspension or Gateway write failures can interrupt these samples; they are not an independent hardware monitor.

The standard launcher retains private per-launch logs in `artifacts/runtime/<UTC timestamp>-<random>/`: `supervisor.log`, `gateway.log`, `mlx-audio.log`, and `frontend.log`. Each session manifest links its starting runtime through `metadata.runtimeLogDirectory`. Gateway restarts append to the same launch logs; a new launcher invocation creates a new directory. Existing external MLX processes and the shared Ollama service have no stdout capture here. These local logs and recordings are ignored by Git; they may contain sermon text and should be treated as private.
...
For viewers using cellular data instead of the venue Wi-Fi, the Firebase Hosting + Realtime Database viewer and outbound-only MacBook publisher are implemented and have a development deployment. When configured, the operator can select the public or LAN QR route. Each route still needs validation on the intended phone/network. See [FIREBASE_PUBLIC_VIEWER.zh.md](./FIREBASE_PUBLIC_VIEWER.zh.md).

With Qwen MLX enabled, the launcher supervises `mlx_audio.server`. If it exits, the launcher records stdout/stderr in `${TMPDIR}/sermon-live-caption-poc/mlx-audio.log` and attempts up to three automatic restarts while the independent browser recording continues. A short MLX finalization timeout is logged as `asr.empty`; repeated timeouts or sustained audio without finals expose a degraded state. A third consecutive identical short ASR result is logged as `asr.suppressed/repeated_short_result`. The `content_words` translation admission guard also retains, but does not translate, a lone closed-class token such as “The.”; its skip is explicit in the log. Neither guard is a speech/music classifier.
...
After a Gateway restart or disconnected socket, keep the recording page open. It tries up to three reconnects once the Gateway is ready; **恢复字幕与保存** retries manually. The browser sends only recovery chunks beyond the durable count and continues the PCM sequence. Missing live PCM is recorded as a gap and padded with silence, so resumed captions never imply uninterrupted translation. The same viewer token survives a restart. Gaps longer than five minutes, changed/torn files, or a previous worker that has not drained require saving the recording and starting a new session. Stop still offers the browser recording and log if server saving fails.

Each session freezes a runtime fingerprint with Git revision, dirty state, model/prompt identity, and selected policies. `LOCAL_LIVE_TRANSLATION_UNIT_POLICY=legacy` remains the default; `bounded_semantic_v1` is an evaluation candidate that joins at most two immutable finals and waits at most 3.2 seconds for narrow unfinished tails. `LOCAL_LIVE_SOURCE_FRAGMENT_POLICY=off` disables the lexical admission guard for comparison. Restart the Gateway after changing either setting. These switches do not change the frozen A0 prompt or create human-reviewed text.
...
- **Current readiness evidence:** [benchmarks/SUNDAY_READINESS_20260904.zh.md](./benchmarks/SUNDAY_READINESS_20260904.zh.md) records the merged-version 60-minute browser WAV replay, Gateway recovery gaps, phone viewport/reconnect checks, semantic A/B decisions, and remaining venue/human-review gates. Browser WAV replay is distinct from the acoustic soak above.
