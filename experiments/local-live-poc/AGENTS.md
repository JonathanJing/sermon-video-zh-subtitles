...
- Preserve the current single-caption UI as the A/B baseline. The recommended next display is a two-tier stack: append-only streaming Chinese for the active bilingual segment, with only the previous final bilingual segment retained above it at a smaller, muted but still readable style. Do not use a horizontal marquee or continuously moving transcript as the default.
