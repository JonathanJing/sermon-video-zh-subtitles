...
export function createFingerprintController({ context, pause, seek, play, onState = () => {}, capture = captureFingerprintAudio, match = matchInWorker, supported = microphoneSupported, now = () => performance.now(), timers = globalThis, expirySeconds = 15 }) {
  let serial = 0, active = null, result = null, expiryTimer = null;
  ...
  async function start() {
    ...
    try {
      ...
      result = { binding, token, end, endedAt: recording.endedAt, confidence: found.confidence };
      ...
      expiryTimer = timers.setTimeout(() => { if (result?.token === token) cancel('expired'); }, (expirySeconds - elapsed) * 1000);
    } catch (error) {
  ...
  function apply() {
    ...
    const elapsed = (now() - item.endedAt) / 1000;
    const target = item.end + elapsed;
    if (!valid(item.token, item.binding) || elapsed < 0 || elapsed > expirySeconds || target >= item.binding.metadata.sourceEndSeconds - item.binding.metadata.sourceStartSeconds || !context().ready) { cancel('expired'); return false; }
    result = null; clearExpiry(); serial++;
    // Exactly one synchronous seek; playback starts in this same click without awaiting IO.
    if (!seek(target)) { emit({ phase: 'error' }); return false; }
    emit({ phase: 'applied', sourceTimeSeconds: target + item.binding.metadata.sourceStartSeconds });
    const appliedToken = serial;
    const failed = () => { if (serial === appliedToken) emit({ phase: 'play_failed' }); };
    try { Promise.resolve(play()).catch(failed); } catch { failed(); }
    return true;
  }
  return { start, apply, cancel, invalidate() { if (active || result) cancel('cancelled'); }, getState: () => state, available: () => Boolean(fingerprintBinding(context())) };
}
...
export function mountFingerprintUI(options) {
  ...
  const messages = {
    idle: '请让设备听到现场正在播放的英文原声，再点击开始。', permission: '等待麦克风许可… 中文已暂停。',
    ...
    expired: '结果已过期或播放内容已切换，请重新定位。', play_failed: '已对齐，但浏览器未开始播放。请点击播放器的播放按钮。',
  };
  const controller = createFingerprintController({ ...options, onState(value) {
    const busy = ['permission', 'starting', 'recovering', 'recording', 'matching'].includes(value.phase);
    $('fingerprint-start').disabled = busy;
    $('fingerprint-start').textContent = value.phase === 'idle' ? '开始听取 10 秒' : '重新听取 10 秒';
    $('fingerprint-apply').hidden = value.phase !== 'matched';
    $('fingerprint-message').textContent = value.phase === 'matched' ? `可靠命中 · 录音末端对应原视频 ${clock(value.sourceTimeSeconds)}。请在 ${Math.max(1, Math.floor(value.expiresInSeconds))} 秒内确认；会补偿等待时间。` : value.phase === 'applied' ? `已对齐至原视频 ${clock(value.sourceTimeSeconds)} 并请求播放。` : value.phase === 'error' && value.diagnostic ? diagnosticMessage(value.diagnostic) : messages[value.phase] || messages.error;
  } });
  ...
  $('fingerprint-apply').addEventListener('click', () => { if (controller.apply()) dialog.close(); });
