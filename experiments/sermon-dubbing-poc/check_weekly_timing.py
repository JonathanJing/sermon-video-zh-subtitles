...
def load_anchors(work, job, job_hash):
    ...
    if approval_path.exists():
        ...
        if not (approval.get("humanApproval") is True and approval.get("reviewedBy") and approval.get("reviewedAt") and approval.get("alignmentSha256") == sha256(alignment_path)):
...
def budgets(blocks, anchors, cues, duration):
    ...
    for i, block in enumerate(blocks):
        ...
        if not anchor or not selected or (i + 1 < len(blocks) and not following):
            ...
            continue
        target_end = following["start"] if following else duration
        natural = selected[-1]["end"] - selected[0]["start"]
        available = target_end - anchor["start"]
        overflow = max(0, natural - available)
        row = {"blockId": block["id"], "videoStart": anchor["start"], "videoEnd": target_end,
