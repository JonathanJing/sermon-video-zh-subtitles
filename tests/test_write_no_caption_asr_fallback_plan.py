...
class WriteNoCaptionAsrFallbackPlanTest(unittest.TestCase):
    def test_plan_documents_real_no_caption_asr_route(self):
        ...
        self.assertEqual(report["requiredModels"]["offlineTranslation"], "gpt-5.4-mini")
        self.assertEqual(report["requiredModels"]["forbiddenOfflineModel"], "gpt-realtime-translate")
        commands = json.dumps(report["commands"])
        runner = json.dumps(report["runnerCommand"])
        self.assertIn("run_no_caption_archive_asr_route.py", runner)
        self.assertIn("--api-key-secret", runner)
        ...
        self.assertEqual(validate_command[validate_command.index("--expected-translation-model") + 1], "gpt-5.4-mini")
        readiness_command = next(command for command in report["commands"] if "scripts/validate_production_readiness.py" in command)
        self.assertIn("--allow-missing-realtime", readiness_command)
