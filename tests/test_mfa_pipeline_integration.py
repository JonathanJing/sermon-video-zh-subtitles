...
class MFAProductionRoutingTests(unittest.TestCase):
    def test_default_reading_routes_to_mfa(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            with patch('scripts.mfa_backend.align_reference_chunks', return_value=[{'source': 'mfa-forced-alignment'}]) as align:
    ...
    def test_mfa_failure_does_not_fall_back_to_estimated_times(self):
        with patch('scripts.mfa_backend.align_reference_chunks', side_effect=RuntimeError('unaligned')):
