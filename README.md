...
**[Four-layer multilingual production: live bilingual workflow](https://jonathanjing.github.io/sermon-video-zh-subtitles/tongxing-video-to-page.html)** — shows each layer's inputs, model/deterministic-tool responsibilities, source-bound audio fingerprint flow, outputs, gates and current implementation boundary.
...
|---|---|---|---|
| 1. Shared English Source & Anchors | Lock the source and sermon window, recover and correct the complete English, align words to source audio, then establish stable sentence, clause, punctuation and pause anchors | `gpt-transcribe` or a trustworthy manuscript supplies text; MFA or Qwen ForcedAligner supplies acoustic word locations; GPT-6 Astra may correct English and review boundaries but must not invent timestamps | `English Source Package`: immutable anchors, word times, provenance, hashes and coverage evidence |
| 2. Target-Language Text | Generate all target-language text directly from English anchors: complete translation, reading edition, companion outline and Scripture-aware revisions | GPT-6 Astra currently performs Chinese translation, two-pass reading edits, companion/outline generation and independent review; deterministic CUV retrieval supplies approved exact Scripture; each future locale chooses its own reviewed adapter | `Target-Language Candidate` plus reviewed reading/companion text: exact English-anchor coverage, model receipts and independent review state |
| 3. Target-Language Audio & Synchronization | Generate natural-rate speech, measure and schedule it against English clauses, then create the source-bound audio fingerprint used for automatic listening alignment | Qwen3-TTS synthesizes audio; Qwen3-ASR back-transcription is a machine screen only; a deterministic scheduler and FFmpeg own timing and assembly; the spectral-landmark fingerprint algorithm is deterministic, not an AI model | `Target-Language Audio Package` plus source-bound fingerprint receipt: audio, captions, schedule, screening, listening and automatic-alignment evidence |
| 4. Multilingual Delivery & Playback | Package and publish approved text/audio/fingerprint assets, then let Web/iOS clients load, match, seek, download and play them | Page/catalog builders, Firebase, Web/iOS clients and validators deliver artifacts; clients match a user-authorized short microphone capture locally; a Supervisor/Agent orchestrates state but does not replace content truth or human acceptance | `Target-Language Release Package`: locale-isolated assets and separate HTTP, fingerprint, device and venue evidence |
...
- Models may be replaced or A/B-tested within a layer, but responsibilities do not blur across layers: a Forced Aligner does not prove semantic completeness, a translation model does not determine acoustic time, back-transcription is not human listening, an audio fingerprint is not speaker-biometric identification, and a successful deployment is not venue acceptance.
